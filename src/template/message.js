exports.run = (client, message) => {
	...
	if (checkRoles(message, cmd) === false) return message.quickEmbed(parseResponses(message, responses.invalidRoles, cmd.conf.neededRoles));
...
function parseResponses(message, resp, permissions) {
    ...
    if (permissions) resp = resp.replace(/%perms%/g, permissions);
    return resp;
}
